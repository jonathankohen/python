...
def register(request):
    ...
    if len(validationErrors) > 0:
        ...
        securedthebag = bcrypt.hashpw(
            request.POST['pw'].encode(), bcrypt.gensalt()).decode()
        newUser = User.objects.create(
            firstName=request.POST['fname'], email=request.POST['email'], password=securedthebag)
...
def createRant(request):
    ...
    newpost = Post.objects.create(content=request.POST['rant'], uploader=User.objects.get(
